...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    if ai:
        texto = FONTE_PONTOS.render(f"Gen: {generation}", 1, (255, 255, 255))
    ...
    while rodando:
        ...
        for i, passaro in enumerate(passaros):
            ...
            if ai:
                ...
                # -1 e 1 -> se o output > 0.5 então o pássaro pula
                if output[0] > 0.5:
                    passaro.pular()
        chao.mover()
        ...
        for i, passaro in enumerate(passaros):
            if (passaro.y + passaro.imagem.get_height()) > chao.y or passaro.y < 0:
                passaros.pop(i)
                if ai:
                    lista_genomas[i].fitness -=1 # pode retirar essa parte depois (penalidade se bater no chão)
                    lista_genomas.pop(i)
                    redes.pop(i)
